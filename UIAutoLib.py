# -*- coding: utf-8 -*-
"""
Created on Thu Jan 23 15:37:24 2020 --- @author: Shultzy

UIAutomationCore for Python 3.
...
#import ST.ctypes.wintypes
#import ST.comtypes.client
TreeNode = Any
...
IsNT6orHigher = ST.os.sys.getwindowsversion().major >= 6
ProcessTime = ST.time.perf_counter  #this returns nearly 0 when first call it if python version <= 3.6
...
class _AutomationClient:
    ...
    def __init__(self):
        try:
            self.UIAutomationCore = ST.comtypes.client.GetModule('UIAutomationCore.dll')
            self.IUIAutomation = ST.comtypes.client.CreateObject("{ff48dba4-60ef-4201-aa87-54103eef594e}", interface=self.UIAutomationCore.IUIAutomation)
            ...
            raise ex
        #Windows dll
        ST.ctypes.windll.user32.GetClipboardData.restype = ST.ctypes.c_void_p
        ST.ctypes.windll.user32.GetWindowDC.restype = ST.ctypes.c_void_p
        ST.ctypes.windll.user32.OpenDesktopW.restype = ST.ctypes.c_void_p
        ST.ctypes.windll.user32.WindowFromPoint.restype = ST.ctypes.c_void_p
        ST.ctypes.windll.user32.SendMessageW.restype = ST.ctypes.wintypes.LONG
        ST.ctypes.windll.user32.GetForegroundWindow.restype = ST.ctypes.c_void_p
        ST.ctypes.windll.user32.GetWindowLongW.restype = ST.ctypes.wintypes.LONG
        ST.ctypes.windll.kernel32.GlobalLock.restype = ST.ctypes.c_void_p
        ST.ctypes.windll.kernel32.GlobalAlloc.restype = ST.ctypes.c_void_p
        ST.ctypes.windll.kernel32.GetStdHandle.restype = ST.ctypes.c_void_p
        ST.ctypes.windll.kernel32.OpenProcess.restype = ST.ctypes.c_void_p
        ST.ctypes.windll.kernel32.CreateToolhelp32Snapshot.restype = ST.ctypes.c_void_p
...
class _DllClient:
    ...
    def __init__(self):
        binPath = ST.os.path.join(ST.os.path.dirname(ST.os.path.abspath(__file__)), "bin")
        ST.os.environ["PATH"] = binPath + ST.os.pathsep + ST.os.environ["PATH"]
        load = False
        if sys.maxsize > 0xFFFFFFFF:
            try:
                self.dll = ST.ctypes.cdll.UIAutomationClient_VC140_X64
                load = True
            except OSError as ex:
                pass
        else:
            try:
                self.dll = ST.ctypes.cdll.UIAutomationClient_VC140_X86
                load = True
            except OSError as ex:
                pass
        if load:
            self.dll.BitmapCreate.restype = ST.ctypes.c_size_t
            self.dll.BitmapFromWindow.argtypes = (ST.ctypes.c_size_t, ST.ctypes.c_int, ST.ctypes.c_int, ST.ctypes.c_int, ST.ctypes.c_int)
            self.dll.BitmapFromWindow.restype = ST.ctypes.c_size_t
            self.dll.BitmapFromFile.argtypes = (ST.ctypes.c_wchar_p, )
            self.dll.BitmapFromFile.restype = ST.ctypes.c_size_t
            self.dll.BitmapToFile.argtypes = (ST.ctypes.c_size_t, ST.ctypes.c_wchar_p, ST.ctypes.c_wchar_p)
            self.dll.BitmapRelease.argtypes = (ST.ctypes.c_size_t, )
            self.dll.BitmapGetWidthAndHeight.argtypes = (ST.ctypes.c_size_t, )
            self.dll.BitmapGetPixel.argtypes = (ST.ctypes.c_size_t, ST.ctypes.c_int, ST.ctypes.c_int)
            self.dll.BitmapSetPixel.argtypes = (ST.ctypes.c_size_t, ST.ctypes.c_int, ST.ctypes.c_int, ST.ctypes.c_uint)
...
class ConsoleScreenBufferInfo(ST.ctypes.Structure):
    _fields_ = [
        ('dwSize', ST.ctypes.wintypes._COORD),
        ('dwCursorPosition', ST.ctypes.wintypes._COORD),
        ('wAttributes', ST.ctypes.c_uint),
        ('srWindow', ST.ctypes.wintypes.SMALL_RECT),
        ('dwMaximumWindowSize', ST.ctypes.wintypes._COORD),
    ]


class MOUSEINPUT(ST.ctypes.Structure):
    _fields_ = (('dx', ST.ctypes.wintypes.LONG),
                ('dy', ST.ctypes.wintypes.LONG),
                ('mouseData', ST.ctypes.wintypes.DWORD),
                ('dwFlags', ST.ctypes.wintypes.DWORD),
                ('time', ST.ctypes.wintypes.DWORD),
                ('dwExtraInfo', ST.ctypes.wintypes.PULONG))

class KEYBDINPUT(ST.ctypes.Structure):
    _fields_ = (('wVk', ST.ctypes.wintypes.WORD),
                ('wScan', ST.ctypes.wintypes.WORD),
                ('dwFlags', ST.ctypes.wintypes.DWORD),
                ('time', ST.ctypes.wintypes.DWORD),
                ('dwExtraInfo', ST.ctypes.wintypes.PULONG))

class HARDWAREINPUT(ST.ctypes.Structure):
    _fields_ = (('uMsg', ST.ctypes.wintypes.DWORD),
                ('wParamL', ST.ctypes.wintypes.WORD),
                ('wParamH', ST.ctypes.wintypes.WORD))

class _INPUTUnion(ST.ctypes.Union):
    _fields_ = (('mi', MOUSEINPUT),
                ('ki', KEYBDINPUT),
                ('hi', HARDWAREINPUT))

class INPUT(ST.ctypes.Structure):
    _fields_ = (('type', ST.ctypes.wintypes.DWORD),
                ('union', _INPUTUnion))


class Rect():
    """
    class Rect, like `ST.ctypes.wintypes.RECT`.
    """
...
_StdOutputHandle = -11
_ConsoleOutputHandle = ST.ctypes.c_void_p(0)
_DefaultConsoleColor = None


def GetClipboardText() -> str:
    if ST.ctypes.windll.user32.OpenClipboard(0):
        if ST.ctypes.windll.user32.IsClipboardFormatAvailable(13): # CF_TEXT=1, CF_UNICODETEXT=13
            hClipboardData = ST.ctypes.windll.user32.GetClipboardData(13)
            hText = ST.ctypes.windll.kernel32.GlobalLock(ST.ctypes.c_void_p(hClipboardData))
            text = ST.ctypes.c_wchar_p(hText).value[:]
            ST.ctypes.windll.kernel32.GlobalUnlock(ST.ctypes.c_void_p(hClipboardData))
            ST.ctypes.windll.user32.CloseClipboard()
            return text
    return ''
...
def SetClipboardText(text: str) -> bool:
    ...
    """
    if ST.ctypes.windll.user32.OpenClipboard(0):
        ST.ctypes.windll.user32.EmptyClipboard()
        textByteLen = (len(text) + 1) * 2
        hClipboardData = ST.ctypes.windll.kernel32.GlobalAlloc(0, textByteLen)  # GMEM_FIXED=0
        hDestText = ST.ctypes.windll.kernel32.GlobalLock(ST.ctypes.c_void_p(hClipboardData))
        ST.ctypes.cdll.msvcrt.wcsncpy(ST.ctypes.c_wchar_p(hDestText), ST.ctypes.c_wchar_p(text), ST.ctypes.c_size_t(textByteLen // 2))
        ST.ctypes.windll.kernel32.GlobalUnlock(ST.ctypes.c_void_p(hClipboardData))
        ...
        ST.ctypes.windll.user32.SetClipboardData(ST.ctypes.c_uint(13), ST.ctypes.c_void_p(hClipboardData))  # CF_TEXT=1, CF_UNICODETEXT=13
        ST.ctypes.windll.user32.CloseClipboard()
        return True
    return False
...
def SetConsoleColor(color: int) -> bool:
    ...
    if not _DefaultConsoleColor:
        if not _ConsoleOutputHandle:
            _ConsoleOutputHandle = ST.ctypes.c_void_p(ST.ctypes.windll.kernel32.GetStdHandle(_StdOutputHandle))
        bufferInfo = ConsoleScreenBufferInfo()
        ST.ctypes.windll.kernel32.GetConsoleScreenBufferInfo(_ConsoleOutputHandle, ST.ctypes.byref(bufferInfo))
        _DefaultConsoleColor = int(bufferInfo.wAttributes & 0xFF)
    if sys.stdout:
        sys.stdout.flush()
    bool(ST.ctypes.windll.kernel32.SetConsoleTextAttribute(_ConsoleOutputHandle, ST.ctypes.c_ushort(color)))
...
def ResetConsoleColor() -> bool:
    ...
    if sys.stdout:
        sys.stdout.flush()
    bool(ST.ctypes.windll.kernel32.SetConsoleTextAttribute(_ConsoleOutputHandle, ST.ctypes.c_ushort(_DefaultConsoleColor)))
...
def WindowFromPoint(x: int, y: int) -> int:
    ...
    """
    return ST.ctypes.windll.user32.WindowFromPoint(ST.ctypes.wintypes.POINT(x, y))  # or ST.ctypes.windll.user32.WindowFromPoint(x, y)
...
def GetCursorPos() -> Tuple[int, int]:
    ...
    """
    point = ST.ctypes.wintypes.POINT(0, 0)
    ST.ctypes.windll.user32.GetCursorPos(ST.ctypes.byref(point))
...
def SetCursorPos(x: int, y: int) -> bool:
    ...
    Return bool, True if succeed otherwise False.
    """
    return bool(ST.ctypes.windll.user32.SetCursorPos(x, y))
...
def PostMessage(handle: int, msg: int, wParam: int, lParam: int) -> bool:
    ...
    Return bool, True if succeed otherwise False.
    """
    return bool(ST.ctypes.windll.user32.PostMessageW(ST.ctypes.c_void_p(handle), ST.ctypes.c_uint(msg), ST.ctypes.wintypes.WPARAM(wParam), ST.ctypes.wintypes.LPARAM(lParam)))
...
def SendMessage(handle: int, msg: int, wParam: int, lParam: int) -> int:
    ...
    """
    return ST.ctypes.windll.user32.SendMessageW(ST.ctypes.c_void_p(handle), ST.ctypes.c_uint(msg), ST.ctypes.wintypes.WPARAM(wParam), ST.ctypes.wintypes.LPARAM(lParam))
...
def GetScreenSize() -> Tuple[int, int]:
    ...
    SM_CXSCREEN = 0
    SM_CYSCREEN = 1
    w = ST.ctypes.windll.user32.GetSystemMetrics(SM_CXSCREEN)
    h = ST.ctypes.windll.user32.GetSystemMetrics(SM_CYSCREEN)
    return w, h
...
def GetMonitorsRect() -> List[Rect]:
    ...
    SM_CMONITORS = 80
    monitorsCount = ST.ctypes.windll.user32.GetSystemMetrics(SM_CMONITORS)
    arrayType = ST.ctypes.c_int * (monitorsCount * 4)
    values = arrayType()
...
def GetPixelColor(x: int, y: int, handle: int = 0) -> int:
    ...
    """
    hdc = ST.ctypes.windll.user32.GetWindowDC(ST.ctypes.c_void_p(handle))
    bgr = ST.ctypes.windll.gdi32.GetPixel(hdc, x, y)
    ST.ctypes.windll.user32.ReleaseDC(ST.ctypes.c_void_p(handle), ST.ctypes.c_void_p(hdc))
    return bgr
...
def MessageBox(content: str, title: str, flags: int = MB.Ok) -> int:
    ...
    """
    return ST.ctypes.windll.user32.MessageBoxW(ST.ctypes.c_void_p(0), ST.ctypes.c_wchar_p(content), ST.ctypes.c_wchar_p(title), ST.ctypes.c_uint(flags))
...
def SetForegroundWindow(handle: int) -> bool:
    ...
    Return bool, True if succeed otherwise False.
    """
    return bool(ST.ctypes.windll.user32.SetForegroundWindow(ST.ctypes.c_void_p(handle)))
...
def BringWindowToTop(handle: int) -> bool:
    ...
    Return bool, True if succeed otherwise False.
    """
    return bool(ST.ctypes.windll.user32.BringWindowToTop(ST.ctypes.c_void_p(handle)))
...
def SwitchToThisWindow(handle: int) -> None:
    ...
    handle: int, the handle of a native window.
    """
    ST.ctypes.windll.user32.SwitchToThisWindow(ST.ctypes.c_void_p(handle), ST.ctypes.c_int(1)) #void function, no return
...
def GetAncestor(handle: int, flag: int) -> int:
    ...
    Return int, a native window handle.
    """
    return ST.ctypes.windll.user32.GetAncestor(ST.ctypes.c_void_p(handle), ST.ctypes.c_int(flag))
...
def IsTopLevelWindow(handle: int) -> bool:
    ...
    """
    return bool(ST.ctypes.windll.user32.IsTopLevelWindow(ST.ctypes.c_void_p(handle)))
...
def GetWindowLong(handle: int, index: int) -> int:
    ...
    index: int.
    """
    return ST.ctypes.windll.user32.GetWindowLongW(ST.ctypes.c_void_p(handle), ST.ctypes.c_int(index))
...
def SetWindowLong(handle: int, index: int, value: int) -> int:
    ...
    """
    return ST.ctypes.windll.user32.SetWindowLongW(ST.ctypes.c_void_p(handle), index, value)
...
def IsIconic(handle: int) -> bool:
    ...
    Return bool.
    """
    return bool(ST.ctypes.windll.user32.IsIconic(ST.ctypes.c_void_p(handle)))
...
def IsZoomed(handle: int) -> bool:
    ...
    Return bool.
    """
    return bool(ST.ctypes.windll.user32.IsZoomed(ST.ctypes.c_void_p(handle)))
...
def IsWindowVisible(handle: int) -> bool:
    ...
    Return bool.
    """
    return bool(ST.ctypes.windll.user32.IsWindowVisible(ST.ctypes.c_void_p(handle)))
...
def ShowWindow(handle: int, cmdShow: int) -> bool:
    ...
    Return bool, True if succeed otherwise False.
    """
    return ST.ctypes.windll.user32.ShowWindow(ST.ctypes.c_void_p(handle), ST.ctypes.c_int(cmdShow))
...
def MoveWindow(handle: int, x: int, y: int, width: int, height: int, repaint: int = 1) -> bool:
    ...
    Return bool, True if succeed otherwise False.
    """
    return bool(ST.ctypes.windll.user32.MoveWindow(ST.ctypes.c_void_p(handle), ST.ctypes.c_int(x), ST.ctypes.c_int(y), ST.ctypes.c_int(width), ST.ctypes.c_int(height), ST.ctypes.c_int(repaint)))
...
def SetWindowPos(handle: int, hWndInsertAfter: int, x: int, y: int, width: int, height: int, flags: int) -> bool:
    ...
    Return bool, True if succeed otherwise False.
    """
    return ST.ctypes.windll.user32.SetWindowPos(ST.ctypes.c_void_p(handle), ST.ctypes.c_void_p(hWndInsertAfter), ST.ctypes.c_int(x), ST.ctypes.c_int(y), ST.ctypes.c_int(width), ST.ctypes.c_int(height), ST.ctypes.c_uint(flags))
...
def GetWindowText(handle: int) -> str:
    ...
    Return str.
    """
    arrayType = ST.ctypes.c_wchar * MAX_PATH
    values = arrayType()
    ST.ctypes.windll.user32.GetWindowTextW(ST.ctypes.c_void_p(handle), values, ST.ctypes.c_int(MAX_PATH))
    return values.value
...
def SetWindowText(handle: int, text: str) -> bool:
    ...
    Return bool, True if succeed otherwise False.
    """
    return bool(ST.ctypes.windll.user32.SetWindowTextW(ST.ctypes.c_void_p(handle), ST.ctypes.c_wchar_p(text)))
...
def GetEditText(handle: int) -> str:
    ...
    arrayType = ST.ctypes.c_wchar * textLen
    values = arrayType()
...
def GetConsoleOriginalTitle() -> str:
    ...
    if IsNT6orHigher:
        arrayType = ST.ctypes.c_wchar * MAX_PATH
        values = arrayType()
        ST.ctypes.windll.kernel32.GetConsoleOriginalTitleW(values, ST.ctypes.c_uint(MAX_PATH))
        return values.value
    else:
...
def GetConsoleTitle() -> str:
    ...
    arrayType = ST.ctypes.c_wchar * MAX_PATH
    values = arrayType()
    ST.ctypes.windll.kernel32.GetConsoleTitleW(values, ST.ctypes.c_uint(MAX_PATH))
    return values.value
...
def SetConsoleTitle(text: str) -> bool:
    ...
    Return bool, True if succeed otherwise False.
    """
    return bool(ST.ctypes.windll.kernel32.SetConsoleTitleW(ST.ctypes.c_wchar_p(text)))
...
def GetForegroundWindow() -> int:
    ...
    """
    return ST.ctypes.windll.user32.GetForegroundWindow()
...
def IsDesktopLocked() -> bool:
    ...
    """
    isLocked = False
    desk = ST.ctypes.windll.user32.OpenDesktopW(ST.ctypes.c_wchar_p('Default'), ST.ctypes.c_uint(0), ST.ctypes.c_int(0), ST.ctypes.c_uint(0x0100))  # DESKTOP_SWITCHDESKTOP = 0x0100
    if desk:
        isLocked = not ST.ctypes.windll.user32.SwitchDesktop(ST.ctypes.c_void_p(desk))
        ST.ctypes.windll.user32.CloseDesktop(ST.ctypes.c_void_p(desk))
    return isLocked
...
def PlayWaveFile(filePath: str = r'C:\Windows\Media\notify.wav', isAsync: bool = False, isLoop: bool = False) -> bool:
    ...
    if filePath:
        ...
        if isLoop:
            flags |= SND_LOOP
            flags |= SND_ASYNC
        return bool(ST.ctypes.windll.winmm.PlaySoundW(ST.ctypes.c_wchar_p(filePath), ST.ctypes.c_void_p(0), ST.ctypes.c_uint(flags)))
    else:
        return bool(ST.ctypes.windll.winmm.PlaySoundW(ST.ctypes.c_wchar_p(0), ST.ctypes.c_void_p(0), ST.ctypes.c_uint(0)))
...
def IsProcess64Bit(processId: int) -> bool:
    ...
    """
    try:
        func = ST.ctypes.windll.ntdll.ZwWow64ReadVirtualMemory64  #only 64 bit OS has this function
    except Exception as ex:
        return False
    try:
        IsWow64Process = ST.ctypes.windll.kernel32.IsWow64Process
    except Exception as ex:
        return False
    hProcess = ST.ctypes.windll.kernel32.OpenProcess(0x1000, 0, processId)  #PROCESS_QUERY_INFORMATION=0x0400,PROCESS_QUERY_LIMITED_INFORMATION=0x1000
    if hProcess:
        is64Bit = ST.ctypes.c_int32()
        if IsWow64Process(ST.ctypes.c_void_p(hProcess), ST.ctypes.byref(is64Bit)):
            ST.ctypes.windll.kernel32.CloseHandle(ST.ctypes.c_void_p(hProcess))
...
def IsUserAnAdmin() -> bool:
    ...
    """
    return bool(ST.ctypes.windll.shell32.IsUserAnAdmin())
...
def RunScriptAsAdmin(argv: List[str], workingDirectory: str = None, showFlag: int = SW.ShowNormal) -> bool:
    ...
    return ST.ctypes.windll.shell32.ShellExecuteW(None, "runas", sys.executable, args, workingDirectory, showFlag) > 32
...
def IsKeyPressed(key: int) -> bool:
    ...
    Return bool.
    """
    state = ST.ctypes.windll.user32.GetAsyncKeyState(key)
...
def SendInput(*inputs) -> int:
    ...
    """
    cbSize = ST.ctypes.c_int(ST.ctypes.sizeof(INPUT))
    for ip in inputs:
        ret = ST.ctypes.windll.user32.SendInput(1, ST.ctypes.byref(ip), cbSize)
    return ret
    #or one call
    #nInputs = len(inputs)
    ...
    #cbSize = ST.ctypes.c_int(ST.ctypes.sizeof(INPUT))
    #return ST.ctypes.windll.user32.SendInput(nInputs, ST.ctypes.byref(pInputs), cbSize)
...
def SendUnicodeChar(char: str, charMode: bool = True) -> int:
    ...
    if charMode:
        ...
        res = ST.ctypes.windll.user32.VkKeyScanW(ST.ctypes.wintypes.WCHAR(char))
        if (res >> 8) & 0xFF == 0:
            vk = res & 0xFF
            scan = 0
            flag = 0
...
def _VKtoSC(key: int) -> int:
    ...
    if key in _SCKeys:
        return _SCKeys[key]
    scanCode = ST.ctypes.windll.user32.MapVirtualKeyA(key, 0)
    if not scanCode:
        return 0
            ...
            if i + 1 == len(keys):
                ...
                if key[1] & KeyboardEventFlag.KeyUp:
                    if keys[i + 1][1] == 'UnicodeChar' or keys[i + 1][1] & KeyboardEventFlag.KeyUp == 0:
                        ...
                        ST.time.sleep(hotkeyInterval)  #must sleep for a while, otherwise combined keys may not be caught
                        if debug:
                            Logger.Write(', sleep({})\n'.format(hotkeyInterval), writeToFile=False)
                else:  #KeyboardEventFlag.KeyDown
                    ST.time.sleep(hotkeyInterval)
                    if debug:
                        Logger.Write(', sleep({})\n'.format(hotkeyInterval), writeToFile=False)
    #make sure hold keys are not pressed
    #win = ST.ctypes.windll.user32.GetAsyncKeyState(Keys.VK_LWIN)
    #ctrl = ST.ctypes.windll.user32.GetAsyncKeyState(Keys.VK_CONTROL)
    #alt = ST.ctypes.windll.user32.GetAsyncKeyState(Keys.VK_MENU)
    #shift = ST.ctypes.windll.user32.GetAsyncKeyState(Keys.VK_SHIFT)
    #if win & 0x8000:
...
class Logger:
    ...
    _SelfFileName = ST.os.path.split(__file__)[1]
    ColorNames = {
    ...
    def Log(log: Any = '', consoleColor: int = -1, writeToFile: bool = True, printToStdout: bool = True, logFile: str = None) -> None:
        ...
        while True:
            ...
            _, scriptFileName = ST.os.path.split(frame.f_code.co_filename)
    ...
    def DeleteLog() -> None:
        """Delete log file."""
        if ST.os.path.exists(Logger.FileName):
            ST.os.remove(Logger.FileName)
...
class Bitmap:
    ...
    def FromHandle(self, hwnd: int, left: int = 0, top: int = 0, right: int = 0, bottom: int = 0) -> bool:
        ...
        self.Release()
        root = GetRootControl()
        rect = ST.ctypes.wintypes.RECT()
        ST.ctypes.windll.user32.GetWindowRect(hwnd, ST.ctypes.byref(rect))
    ...
    def FromFile(self, filePath: str) -> bool:
        ...
        """
        self.Release()
        self._bitmap = _DllClient.instance().dll.BitmapFromFile(ST.ctypes.c_wchar_p(filePath))
        self._getsize()
        return self._bitmap > 0
    ...
    def ToFile(self, savePath: str) -> bool:
        ...
        Return bool, True if succeed otherwise False.
        """
        name, ext = ST.os.path.splitext(savePath)
        ...
        return bool(_DllClient.instance().dll.BitmapToFile(self._bitmap, ST.ctypes.c_wchar_p(savePath), ST.ctypes.c_wchar_p(gdiplusImageFormat)))
    ...
    def GetPixelColorsHorizontally(self, x: int, y: int, count: int) -> ST.ctypes.Array:
        """
        x: int.
        y: int.
        count: int.
        Return `ST.ctypes.Array`, an iterable array of int values in argb form point x,y horizontally.
        """
        arrayType = ST.ctypes.c_uint32 * count
        values = arrayType()
        _DllClient.instance().dll.BitmapGetPixelsHorizontally(ST.ctypes.c_size_t(self._bitmap), x, y, values, count)
        return values
    ...
    def SetPixelColorsHorizontally(self, x: int, y: int, colors: Iterable[int]) -> bool:
        ...
        return _DllClient.instance().dll.BitmapSetPixelsHorizontally(ST.ctypes.c_size_t(self._bitmap), x, y, values, count)

    def GetPixelColorsVertically(self, x: int, y: int, count: int) -> ST.ctypes.Array:
        """
        x: int.
        y: int.
        count: int.
        Return `ST.ctypes.Array`, an iterable array of int values in argb form point x,y vertically.
        """
        arrayType = ST.ctypes.c_uint32 * count
        values = arrayType()
        _DllClient.instance().dll.BitmapGetPixelsVertically(ST.ctypes.c_size_t(self._bitmap), x, y, values, count)
        return values
    ...
    def SetPixelColorsVertically(self, x: int, y: int, colors: Iterable[int]) -> bool:
        ...
        arrayType = ST.ctypes.c_uint32 * count
        values = arrayType(*colors)
        return _DllClient.instance().dll.BitmapSetPixelsVertically(ST.ctypes.c_size_t(self._bitmap), x, y, values, count)

    def GetPixelColorsOfRow(self, y: int) -> ST.ctypes.Array:
        """
        y: int, row index.
        Return `ST.ctypes.Array`, an iterable array of int values in argb of y row.
        """
        return self.GetPixelColorsOfRect(0, y, self.Width, 1)

    def GetPixelColorsOfColumn(self, x: int) -> ST.ctypes.Array:
        """
        x: int, column index.
        Return `ST.ctypes.Array`, an iterable array of int values in argb of x column.
        """
        return self.GetPixelColorsOfRect(x, 0, 1, self.Height)

    def GetPixelColorsOfRect(self, x: int, y: int, width: int, height: int) -> ST.ctypes.Array:
        """
        x: int.
        y: int.
        width: int.
        height: int.
        Return `ST.ctypes.Array`, an iterable array of int values in argb of the input rect.
        """
        arrayType = ST.ctypes.c_uint32 * (width * height)
        values = arrayType()
        _DllClient.instance().dll.BitmapGetPixelsOfRect(ST.ctypes.c_size_t(self._bitmap), x, y, width, height, values)
        return values
    ...
    def SetPixelColorsOfRect(self, x: int, y: int, width: int, height: int, colors: Iterable[int]) -> bool:
        ...
        arrayType = ST.ctypes.c_uint32 * (width * height)
        values = arrayType(*colors)
        return bool(_DllClient.instance().dll.BitmapSetPixelsOfRect(ST.ctypes.c_size_t(self._bitmap), x, y, width, height, values))

    def GetPixelColorsOfRects(self, rects: List[Tuple[int, int, int, int]]) -> List[ST.ctypes.Array]:
        """
        rects: List[Tuple[int, int, int, int]], such as [(0,0,10,10), (10,10,20,20), (x,y,width,height)].
        Return List[ST.ctypes.Array], a list whose elements are ST.ctypes.Array which is an iterable array of int values in argb.
        """
    ...
    def GetAllPixelColors(self) -> ST.ctypes.Array:
        """
        Return `ST.ctypes.Array`, an iterable array of int values in argb.
        """
...
def GetPatternIdInterface(patternId: int):
    ...
    Return ST.comtypes._cominterface_meta.
    """
...
class AnnotationPattern():
    ...
    def DateTime(self) -> str:
        """
        Property DateST.time.
        Call IUIAutomationAnnotationPattern::get_CurrentDateST.time.
...
class ObjectModelPattern():
    ...
    def GetUnderlyingObjectModel(self) -> ST.ctypes.POINTER(ST.comtypes.IUnknown):
        """
        Call IUIAutomationObjectModelPattern::GetUnderlyingObjectModel, todo.
        Return `ST.ctypes.POINTER(ST.comtypes.IUnknown)`, an interface used to access the underlying object model of the provider.
...
class SelectionPattern():
    ...
    def CanSelectMultiple(self) -> bool:
        ...
        Return bool, indicates whether more than one item in the container can be selected at one ST.time.
...
class TextRange():
    ...
    def GetAttributeValue(self, textAttributeId: int) -> ST.ctypes.POINTER(ST.comtypes.IUnknown):
        """
        Call IUIAutomationTextRange::GetAttributeValue.
        textAttributeId: int, a value in class `TextAttributeId`.
        Return `ST.ctypes.POINTER(ST.comtypes.IUnknown)` or None, the value of the specified text attribute across the entire text range, todo.
...
class TextPattern():
    ...
    def RangeFromPoint(self, x: int, y: int) -> TextRange:
        ...
        """
        textRange = self.pattern.RangeFromPoint(ST.ctypes.wintypes.POINT(x, y))
        if textRange:
            return TextRange(textRange=textRange)
...
def CreatePattern(patternId: int, pattern: ST.ctypes.POINTER(ST.comtypes.IUnknown)):
...
class Control():
    ...
    def __init__(self, searchFromControl: 'Control' = None, searchDepth: int = 0xFFFFFFFF, searchInterval: float = SEARCH_INTERVAL, foundIndex: int = 1, element=None, **searchProperties):
        ...
        element: `ST.ctypes.POINTER(IUIAutomationElement)`, internal use only.
        searchProperties: defines how to search, the following keys can be used:
                            ...
                            RegexName: str, supports regex using ST.re.match.
        ...
        self.regexName = ST.re.compile(regName) if regName else None
    ...
    def CreateControlFromElement(element) -> 'Control':
        ...
        element: `ST.ctypes.POINTER(IUIAutomationElement)`.
    ...
    def GetPattern(self, patternId: int):
        ...
        try:
            ...
            if pattern:
                ...
                return subPattern
        except ST.comtypes.COMError as ex:
            pass
    ...
    def Element(self):
        """
        Property Element.
        Return `ST.ctypes.POINTER(IUIAutomationElement)`.
        """
            ...
            if control:
                ...
                if DEBUG_SEARCH_TIME:
                    Logger.ColorfullyLog('{} TraverseControls: <Color=Cyan>{}</Color>, SearchTime: <Color=Cyan>{:.3f}</Color>s[{} - {}]'.format(
                        ...
                        startDateTime.time.time(), ST.dt.now().time()))
                return True
            else:
                remain = startTime + maxSearchSeconds - ProcessTime()
                if remain > 0:
                    ST.time.sleep(min(remain, searchIntervalSeconds))
                else:
                    if printIfNotExist or DEBUG_EXIST_DISAPPEAR:
                        Logger.ColorfullyLog(self.GetColorfulSearchPropertiesStr() + '<Color=Red> does not exist.</Color>')
                    return False
...
def WalkTree(top, getChildren: Callable[[TreeNode], List[TreeNode]] = None,
    ...
    def GetDirChildren(dir_):
        if ST.os.path.isdir(dir_):
            return [ST.os.path.join(dir_, it) for it in ST.os.listdir(dir_)]
...
def GetConsoleWindow() -> WindowControl:
    ...
    return ControlFromHandle(ST.ctypes.windll.kernel32.GetConsoleWindow())
...
def ControlFromPoint(x: int, y: int) -> Control:
    ...
    Return `Control` subclass or None.
    """
    element = _AutomationClient.instance().IUIAutomation.ElementFromPoint(ST.ctypes.wintypes.POINT(x, y))
...
def NotePadLogControl(control: Control, maxDepth: int = 0xFFFFFFFF, showAllName: bool = True, startDepth: int = 0) -> None:
    """
    ...
    """
    try:
        logtxt = open('C:\Temp\TreeLog.txt', 'w')
        for c, d in WalkControl(control, True, maxDepth):
            indent = d * 4
            iTitle = ST.Utilities.IndentTxt("|--- " + str(d) + " ---|", indent)
            iName = ST.Utilities.IndentTxt("Name:   " + str(c.Name), indent)
            iClassName = ST.Utilities.IndentTxt("ClassName:   " + str(c.ClassName), indent)
            iAutoID = ST.Utilities.IndentTxt("AutoID:   " + str(c.AutomationId), indent)
            iControlType = ST.Utilities.IndentTxt("ControlType:   " + str(c.ControlTypeName), indent)
            iHandle = ST.Utilities.IndentTxt("Handle:   " + str(c.NativeWindowHandle), indent)
            iNoHandle = ST.Utilities.IndentTxt('No Handle Found', indent)
            iHelpTxt = ST.Utilities.IndentTxt("HelpTxt:   " + str(c.HelpText), indent)
            iDescription = ST.Utilities.IndentTxt("Description:   " + str(c.ProviderDescription), indent)
            
            logtxt.write(iTitle + "\n")
            logtxt.write(iName + "\n")
            logtxt.write(iClassName + "\n")
            logtxt.write(iAutoID + "\n")
            logtxt.write(iControlType + "\n")
            if str(c.NativeWindowHandle) == 0:
                logtxt.write(iNoHandle + "\n")
            else:
                logtxt.write(iHandle + "\n")
            logtxt.write(iHelpTxt + "\n")
            logtxt.write(iDescription + "\n")
        logtxt.close
    except Exception as Ex:
        ST.ErrorLogList.append("UIAutoLib --> EnumNotePadLogControl --> WalkTree - Element Error:  " + str(Ex))

def NotePadElementLogger(ElementList: []) -> None:
    """
    Print and log control and its descendants' propertyies.
    control: `Control` or its subclass.
    maxDepth: int, enum depth.
    ...
    try:
        logtxt = open('C:\Temp\ElementLog.txt', 'w')
        for c in ElementList:
            i = ElementList.index(c)
            indent = i * 4
            iTitle = ST.Utilities.IndentTxt("|--- " + str(i) + " ---|", indent)
            iName = ST.Utilities.IndentTxt("Name:   " + str(c.Name), indent)
            iClassName = ST.Utilities.IndentTxt("ClassName:   " + str(c.ClassName), indent)
            iAutoID = ST.Utilities.IndentTxt("AutoID:   " + str(c.AutomationId), indent)
            iControlType = ST.Utilities.IndentTxt("ControlType:   " + str(c.ControlTypeName), indent)
...
#            logtxt.write(iDescription + "\n")
        ...
        ST.ErrorLogList.append("UIAutoLib --> NotePadElementLogger --> Element List Logger Error:  " + str(Ex))

def NotePadLogParents(ElementList: []):
    """
    ...
    """
    lists = []
    logtxt = open('C:\Temp\ParentsTreeLog.txt', 'w')
    try:
        for ele in ElementList:
            ...
            while control:
                ...
                for i, control in enumerate(lists):
                    indent = i * 4
                    iTitle = ST.Utilities.IndentTxt("|--- " + str(i) + " ---|", indent)
                    iName = ST.Utilities.IndentTxt("Name:   " + str(control.Name), indent)
                    iClassName = ST.Utilities.IndentTxt("ClassName:   " + str(control.ClassName), indent)
                    iAutoID = ST.Utilities.IndentTxt("AutoID:   " + str(control.AutomationId), indent)
                    iControlType = ST.Utilities.IndentTxt("ControlType:   " + str(control.ControlTypeName), indent)
                    iHandle = ST.Utilities.IndentTxt("Handle:   " + str(control.NativeWindowHandle), indent)
                    iNoHandle = ST.Utilities.IndentTxt("No Handle Found:   " + str(control.NativeWindowHandle), indent)
                    iHelpTxt = ST.Utilities.IndentTxt("HelpTxt:   " + str(control.HelpText), indent)
#                    iDescription = ST.Utilities.IndentTxt("Description:   " + str(control.ProviderDescription), indent)
                    ...
                    logtxt.write(iAutoID + "\n")
                    logtxt.write(iControlType + "\n")
                    if str(control.NativeWindowHandle) == 0:
                        logtxt.write(iNoHandle + "\n")
                    else:
                        logtxt.write(iHandle + "\n")
                    logtxt.write(iHelpTxt + "\n")
#                    logtxt.write(iDescription + "\n")
        ...
        ST.ErrorLogList.append("UIAutoLib --> NotePadLogParents --> GetParents - Element Error:  " + str(Ex))
    logtxt.close
...
def RunByHotKey(keyFunctions: Dict[Tuple[int, int], Callable], stopHotKey: Tuple[int, int] = None, exitHotKey: Tuple[int, int] = (ModifierKey.Control, Keys.VK_D), waitHotKeyReleased: bool = True) -> None:
    ...
    for hotkey in keyFunctions:
        ...
        if ST.ctypes.windll.user32.RegisterHotKey(0, hotKeyId, hotkey[0], hotkey[1]):
    ...
    if stopHotKey and len(stopHotKey) == 2:
        ...
        if ST.ctypes.windll.user32.RegisterHotKey(0, stopHotKeyId, stopHotKey[0], stopHotKey[1]):
    ...
    if exitHotKey and len(exitHotKey) == 2:
        ...
        if ST.ctypes.windll.user32.RegisterHotKey(0, exitHotKeyId, exitHotKey[0], exitHotKey[1]):
    ...
    livingThreads = []
    stopEvent = Event()
    msg = ST.ctypes.wintypes.MSG()
    while ST.ctypes.windll.user32.GetMessageW(ST.ctypes.byref(msg), ST.ctypes.c_void_p(0), ST.ctypes.c_uint(0), ST.ctypes.c_uint(0)) != 0:
...
#        except (KeyError, ST.comtypes.COMError) as ex:
